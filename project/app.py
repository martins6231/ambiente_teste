    ...
    def _load_translations(self) -> Dict[str, Dict[str, str]]:
        ...
        return {
            ...
            "en": {
                ...
                "not_valid_excel": "Downloaded file is not a valid Excel.",
...
class Dashboard:
    ...
    def initialize_session_state(self):
        ...
        if st.session_state.first_run:
            ...
            st.session_state.category = self.df['categoria'].unique()[0]
            st.session_state.years = sorted(self.df['data'].dt.year.unique())
            st.session_state.months = [f"{m:02d} - {calendar.month_abbr[m]}" for m in range(1, 13)]
            st.session_state.use_date_range = False
            st.session_state.start_date = self.df['data'].min()
            st.session_state.end_date = self.df['data'].max()
